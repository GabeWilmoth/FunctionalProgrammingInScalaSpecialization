...
trait Solver extends GameDef:
   ...
   */
  def done(b: Block): Boolean = b.b1 == goal && b.b2 == goal && b.isStanding
   ...
   */
  def neighborsWithHistory(b: Block, history: List[Move]): LazyList[(Block, List[Move])] = b
      .legalNeighbors
  ...
  def newNeighborsOnly(neighbors: LazyList[(Block, List[Move])],
                       explored: Set[Block]): LazyList[(Block, List[Move])] = neighbors filter ((block, _) => !explored.contains(block))
  ...
  def from(initial: LazyList[(Block, List[Move])],
           explored: Set[Block]): LazyList[(Block, List[Move])] = if   initial.isEmpty
    then LazyList()
    else
   ...
   */
  lazy val pathsToGoal: LazyList[(Block, List[Move])] = pathsFromStart filter ((block, _) => done(block))
   ...
   * position.
   */
  lazy val solution: List[Move] = if   pathsToGoal.isEmpty
    then List[Move]()
    else
      pathsToGoal
        .minBy(_._2.length)
