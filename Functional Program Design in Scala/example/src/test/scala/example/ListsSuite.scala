...
class ListsSuite extends munit.FunSuite:
  ...
  test("details why one plus one is not three (0pts)") {
    assertEquals(1 + 2, 3) // Fix me, please!
  }
  ...
  test("sum with negative numbers (10pts)") {
    assert(sum(List(1,-2)) == -1)
  }

  test("sum with 0 as input (10pts)") {
    assert(sum(List(1,0)) == 1)
  }

  test("sum with empty list (10pts)") {
    assert(sum(List()) == 0)
  }
  ...
  test("max with negative numbers (10pts)") {
    assert(max(List(3, -7, -2)) == 3)
  }

  test("max with 0 as input (10pts)") {
    assert(max(List(0)) == 0)
  }
