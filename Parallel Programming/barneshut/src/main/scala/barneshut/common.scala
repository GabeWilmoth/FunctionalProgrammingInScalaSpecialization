...
case class Fork(
  ...
  val weightX = nw.mass * nw.massX + ne.mass * ne.massX + sw.mass * sw.massX + se.mass * se.massX
  val weightY = nw.mass * nw.massY + ne.mass * ne.massY + sw.mass * sw.massY + se.mass * se.massY

  val massX: Float = if (mass == 0) then centerX else weightX / mass
  val massY: Float = if (mass == 0) then centerY else weightY / mass
...
extends Quad:
  ...
  val weightX = bodies.foldLeft(0f)((m, body) => m + body.mass * body.x)
  val weightY = bodies.foldLeft(0f)((m, body) => m + body.mass * body.y)
  val massCal: Float = bodies.foldLeft(0f)((m, body) => m + body.mass)
  
  val (mass, massX, massY) = (massCal, weightX / massCal, weightY / massCal)
...
class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) extends SectorMatrixInterface:
  ...
  def +=(b: Body): SectorMatrix = {
    ...
    val sec2X = (secX - boundaries.minX) / (boundaries.maxX - boundaries.minX) * sectorPrecision
    val sec2Y = (secY - boundaries.minY) / (boundaries.maxY - boundaries.minY) * sectorPrecision

    matrix(sec2Y.toInt * sectorPrecision + sec2X.toInt) += b
