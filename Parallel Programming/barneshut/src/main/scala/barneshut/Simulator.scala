...
class Simulator(val taskSupport: TaskSupport, val timeStats: TimeStatistics):
  ...
  def updateBoundaries(boundaries: Boundaries, body: Body): Boundaries = {
    ...
    newBd.minX = math.min(body.x, boundaries.minX)
    newBd.maxX = math.max(body.x, boundaries.maxX)
    newBd.minY = math.min(body.y, boundaries.minY)
    newBd.maxY = math.max(body.y, boundaries.maxY)
    newBd
  }

  def mergeBoundaries(a: Boundaries, b: Boundaries): Boundaries = {
    val newBd: Boundaries = new Boundaries
    newBd.minX = math.min(a.minX, b.minX)
