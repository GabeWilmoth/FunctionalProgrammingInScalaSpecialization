...
object ParallelCountChange extends ParallelCountChangeInterface:
  ...
  def countBaseCase(money: Int, coins: List[Int]): Int = {
    if (money == 0) {
      1
  ...
  def countChange(money: Int, coins: List[Int]): Int = {
    ...
    then countBaseCase(money, coins)
  ...
  def moneyThreshold(startingMoney: Int): Threshold = (money: Int, _) => money <= (startingMoney * 2.0 / 3.0)

  /** Threshold heuristic based on the total number of initial coins. */
  def totalCoinsThreshold(totalCoins: Int): Threshold = (_, coins: List[Int]) => coins.size <= (totalCoins * 2.0 / 3.0)


  /** Threshold heuristic based on the starting money and the initial list of coins. */
  def combinedThreshold(startingMoney: Int, allCoins: List[Int]): Threshold = (money: Int, coins: List[Int]) => money * coins.size <= (startingMoney * allCoins.size / 2.0)