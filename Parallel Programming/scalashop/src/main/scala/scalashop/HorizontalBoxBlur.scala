...
object HorizontalBoxBlur extends HorizontalBoxBlurInterface:
  ...
  def parBlur(src: Img, dst: Img, numTasks: Int, radius: Int): Unit = {
    val nodes = if (numTasks > src.height) src.height else src.height / numTasks
    val strips: List[Seq[Int]] = (0 until src.height).grouped(nodes).toList
    val processes = strips.map(s =>{
      task {
        blur(src, dst, s.head, s.last + 1, radius)
      }
    })
