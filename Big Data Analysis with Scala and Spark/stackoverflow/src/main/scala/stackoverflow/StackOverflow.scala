...
class StackOverflow extends StackOverflowInterface with Serializable:
  ...
  def scoredPostings(grouped: RDD[(QID, Iterable[(Question, Answer)])]): RDD[(Question, HighScore)] =
    ...
    val groupedA = grouped.mapValues(qAndA => {
      val answers = qAndA.map{ case (question, answer) => answer}
      answers.toArray
    })
    val highIds = groupedA.mapValues(answerHighScore)
    val joined = grouped.join(highIds)
    val qAndS = joined.map{ case (id, (qAndA, highScore)) =>
      (qAndA.head._1, highScore)
    }
    qAndS
