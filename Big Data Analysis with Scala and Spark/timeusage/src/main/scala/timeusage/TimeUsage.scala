...
object TimeUsage extends TimeUsageInterface:
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) =
    val primaryNeedsInitials = List("t01", "t03", "t11", "t1801", "t1803")
    val workInitials = List("t05", "t1805")
    val otherInitials = List("t02", "t04", "t06", "t07", "t08", "t09", "t10", "t12", "t13", "t14", "t15", "t16", "t18")
    val columnNamesGrouped = columnNames.groupBy(columnName =>
      if (primaryNeedsInitials.exists(initial => columnName.startsWith(initial))) {
        "primaryNeeds"
      } else if (workInitials.exists(initial => columnName.startsWith(initial))) {
        "work"
      } else if (otherInitials.exists(initial => columnName.startsWith(initial))) {
        "other"
      } else {
        "unused"
      }
    )
    val primaryNeedsColumns = columnNamesGrouped("primaryNeeds").map(columnName => col(columnName))
    val workingColumns = columnNamesGrouped("work").map(columnName => col(columnName))
    val otherColumns = columnNamesGrouped("other").map(columnName => col(columnName))
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] =
    summed.groupByKey(timeUsageRow => (timeUsageRow.working, timeUsageRow.sex, timeUsageRow.age))
      ...
      .select($"key._1" as "working", $"key._2" as "sex", $"key._3" as "age", $"primaryNeeds", $"work", $"other")
      .orderBy($"working", $"sex", $"age")
      .as[TimeUsageRow]
